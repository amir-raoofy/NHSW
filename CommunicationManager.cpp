	...
	if (parameters_.topology.right_id!=-1)  {
		...
		for (int i = 0; i < parameters_.get_num_cells(1)+2; i++) {
			for (int k = 0; k < parameters_.get_num_cells(2); k++) {
				right_send_buffer_rectangle[i]=field[parameters_.get_num_cells(0)][i][k];
			}
		}
		MPI::COMM_WORLD.Send(right_send_buffer_rectangle, parameters_.get_num_cells(1)+2, MPI::DOUBLE, parameters_.topology.right_id, 0);
	}

	if (parameters_.topology.left_id!=-1) {
		MPI::COMM_WORLD.Recv(left_recv_buffer_rectangle, parameters_.get_num_cells(1)+2, MPI::DOUBLE, parameters_.topology.left_id, 0);
		//read the buffer
		for (int i = 0; i < parameters_.get_num_cells(1)+2; i++) {
			for (int k = 0; k < parameters_.get_num_cells(2); k++) {
				field[0][i][k]=left_recv_buffer_rectangle[i];
			}
		}
	}
...
void CommunicationManager::updateLeftNeighbour(DiscreteCube &field){
	...
	if (parameters_.topology.left_id!=-1)  {
		...
		for (int i = 0; i < parameters_.get_num_cells(1)+2; i++) {
			for (int k = 0; k < parameters_.get_num_cells(2); k++) {
				left_send_buffer_rectangle[i]=field[1][i][k];
			}
		}
		MPI::COMM_WORLD.Send(left_send_buffer_rectangle, parameters_.get_num_cells(1)+2, MPI::DOUBLE, parameters_.topology.left_id, 0);
	}

	if (parameters_.topology.right_id!=-1) {
		MPI::COMM_WORLD.Recv(right_recv_buffer_rectangle, parameters_.get_num_cells(1)+2, MPI::DOUBLE, parameters_.topology.right_id, 0);
		//read the buffer
		for (int i = 0; i < parameters_.get_num_cells(1)+2; i++) {
			for (int k = 0; k < parameters_.get_num_cells(2); k++) {
				field[parameters_.get_num_cells(0)+1][i][k]=right_recv_buffer_rectangle[i];
			}
		}
	}
...
void CommunicationManager::updateFrontNeighbour(DiscreteCube &field){
	...
	if (parameters_.topology.front_id!=-1)  {
		...
		for (int i = 0; i < parameters_.get_num_cells(0)+2; i++) {
			for (int k = 0; k < parameters_.get_num_cells(2); k++) {
				front_send_buffer_rectangle[i]=field[i][parameters_.get_num_cells(1)][k];
			}
		}
		MPI::COMM_WORLD.Send(front_send_buffer_rectangle, parameters_.get_num_cells(0)+2, MPI::DOUBLE, parameters_.topology.front_id, 0);
	}

	if (parameters_.topology.back_id!=-1) {
		MPI::COMM_WORLD.Recv(back_recv_buffer_rectangle, parameters_.get_num_cells(0)+2, MPI::DOUBLE, parameters_.topology.back_id, 0);
		//read the buffer
		for (int i = 0; i < parameters_.get_num_cells(0)+2; i++) {
			for (int k = 0; k < parameters_.get_num_cells(2); k++) {
				field[i][0][k]=back_recv_buffer_rectangle[i];
			}
		}
	}
...
void CommunicationManager::updateBackNeighbour(DiscreteCube &field){
	...
	if (parameters_.topology.back_id!=-1)  {
		...
		for (int i = 0; i < parameters_.get_num_cells(0)+2; i++) {
			for (int k = 0; k < parameters_.get_num_cells(2); k++) {
				back_send_buffer_rectangle[i]=field[i][1][k];
			}
		}
		MPI::COMM_WORLD.Send(back_send_buffer_rectangle, parameters_.get_num_cells(0)+2, MPI::DOUBLE, parameters_.topology.back_id, 0);
	}

	if (parameters_.topology.front_id!=-1) {
		MPI::COMM_WORLD.Recv(front_recv_buffer_rectangle, parameters_.get_num_cells(0)+2, MPI::DOUBLE, parameters_.topology.front_id, 0);
		//read the buffer
		for (int i = 0; i < parameters_.get_num_cells(0)+2; i++) {
			for (int k = 0; k < parameters_.get_num_cells(2); k++) {
				field[i][parameters_.get_num_cells(1)+1][k]=front_recv_buffer_rectangle[i];
			}
		}
	}
